...
class DB:
    ...
    def init(self):
        with lock:
            ...
            # Agar eski REQUIRED_CHANNELS bo'lsa, ularni forced_channels ga o'tkazish (ixtiyoriy)
            self.cur.execute("SELECT COUNT(*) FROM forced_channels")
            if self.cur.fetchone()[0] == 0:
                # Default kanallarni qo'shish (agar mavjud bo'lsa)
                default_channels = [
    ...
    # ============= Majburiy kanallar (forced channels) =============
    ...
    def complete_task(self, user_id, task_id, reward):
        with lock:
            # Faqat bir marta bajarish mumkin
...
def check_sub(uid):
    ...
    for ch_id, username, name, url in channels:
        try:
            ...
            # Bot tekshira olmasa (kanalda admin emas yoki boshqa xato) -> obuna bo'lgan deb hisoblaymiz
...
"""
        ...
        for tid, ch_id, username, name, url, reward in tasks:
            if tid in completed:
                status = "✅ Bajarilgan"
                btn_text = f"{status} {name} (+{reward}⭐)"
                markup.add(types.InlineKeyboardButton(btn_text, callback_data=f"task_{tid}_done"))
            else:
                btn_text = f"📢 {name} - obuna bo'ling (+{reward}⭐)"
        ...
        text = "✅ <b>VAZIFALAR</b>\n\nKanalga obuna bo'ling va +0.25⭐ oling!\nHar bir vazifani faqat bir marta bajarish mumkin.\n\n👇 Obuna bo'lgach, 'Tekshirish' tugmasini bosing:"
        bot.send_message(call.message.chat.id, text, reply_markup=markup)
    
    elif data.startswith("task_"):
        # Vazifa tekshiruvi: task_123_check
        parts = data.split("_")
            ...
            try:
                ...
                if member.status in ['member', 'administrator', 'creator']:
                    if db.complete_task(uid, tid, reward):
                        ...
                        # Yangilangan vazifalar ro'yxatini ko'rsatish
                        callback_data = "tasks"
                        callback(call)
                    else:
                ...
                # Tekshirib bo'lmasa, obuna bo'lgan deb hisoblash (xavfsizlik)
...
def tasks_cmd(m):
    ...
    markup.add(types.InlineKeyboardButton("🔁 Tekshirish", switch_inline_query_current_chat=""))  # placeholder
    # Aslida tekshirish uchun alohida tugma kerak, lekin callback bilan ishlaymiz
    # Vaqtinchalik: har bir vazifa uchun alohida tekshirish tugmasi qo'shamiz
    markup2 = types.InlineKeyboardMarkup(row_width=2)
    for tid, ch_id, username, name, url, reward in tasks:
        if tid not in completed:
            markup2.add(types.InlineKeyboardButton(f"🔍 {name}", callback_data=f"task_{tid}_check"))
    if markup2.keyboard:
        bot.reply_to(m, "✅ Vazifalar: Kanalga obuna bo'ling, so'ng 'Tekshirish' tugmasini bosing.", reply_markup=markup2)
    else:
...
if __name__ == "__main__":
    ...
    print("📢 Majburiy kanallar va vazifalar admin tomonidan boshqariladi")
    print("=" * 50)
