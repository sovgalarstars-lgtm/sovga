import os, time, random, logging, sqlite3, requests, threading
...
class DB:
    def __init__(self):
        if DATABASE_URL:
            try:
                ...
                print(f"❌ PostgreSQL ulanib bo'lmadi ({e}). SQLite ga o'tilmoqda...")
    ...
    def create_user(self, uid, username, name):
        self._execute(
            "INSERT INTO users (user_id, username, first_name) VALUES (?,?,?) ON CONFLICT (user_id) DO NOTHING",
            (uid, username, name)
        )

    def get(self, uid):
        row = self._execute(
            "SELECT successful_invites, stars, vip, total_spent, last_daily, daily_streak, total_earned FROM users WHERE user_id=?",
            (uid,), fetch="one"
        )
        if row:
            return {
                "successful_invites": row[0] or 0,
                "stars": float(row[1] or 0),
                "vip": row[2] or 0,
                "spent": float(row[3] or 0),
                "last_daily": row[4],
                "streak": row[5] or 0,
                "earned": float(row[6] or 0)
            }
    ...
    def add_pending_referral(self, invited_id, inviter_id):
        self._execute(
            "INSERT INTO pending_referrals (invited_id, inviter_id) VALUES (?,?) ON CONFLICT (invited_id) DO UPDATE SET inviter_id=?",
            (invited_id, inviter_id, inviter_id)
        )
    ...
    def add_history(self, inviter_id, invited_id, invited_name, source="link"):
        self._execute(
            "INSERT INTO invite_history (inviter_id, invited_id, invited_name, source) VALUES (?,?,?,?)",
            (inviter_id, invited_id, invited_name, source)
        )

    def check_duplicate(self, inviter_id, invited_id):
        row = self._execute(
            "SELECT COUNT(*) FROM invite_history WHERE inviter_id=? AND invited_id=?",
            (inviter_id, invited_id), fetch="one"
        )
        return row[0] > 0

    def add_successful_invite(self, uid):
        row = self._execute(
            "SELECT successful_invites, stars, total_earned FROM users WHERE user_id=?",
            (uid,), fetch="one"
        )
        if row:
        ...
        self._execute(
            "UPDATE users SET successful_invites=?, stars=?, total_earned=? WHERE user_id=?",
            (new_cnt, new_stars, new_earned, uid)
        )
    ...
    def give_daily_bonus(self, uid):
        row = self._execute(
            "SELECT last_daily, stars, daily_streak, total_earned FROM users WHERE user_id=?",
            (uid,), fetch="one"
        )
        if not row:
        ...
        ne = te + bonus
        self._execute(
            "UPDATE users SET stars=?, last_daily=?, daily_streak=?, total_earned=? WHERE user_id=?",
            (ns, now.isoformat(), streak, ne, uid)
        )
    ...
    def get_purchase_history(self, uid):
        return self._execute(
            "SELECT item_name, item_emoji, price, created_at FROM purchase_history WHERE user_id=? ORDER BY created_at DESC LIMIT 10",
            (uid,), fetch="all"
        )

    def add_purchase_history(self, uid, name, emoji, price):
        self._execute(
            "INSERT INTO purchase_history (user_id, item_name, item_emoji, price) VALUES (?,?,?,?)",
            (uid, name, emoji, price)
        )
    ...
    def get_stats(self):
        ...
        return {"users": users, "total_invites": total_invites, "stars": stars, "vip": vip, "spent": spent, "purchases": purchases}

    # ---------- Majburiy kanallar ----------
    def get_forced_channels(self):
        return self._execute(
            "SELECT id, channel_type, channel_id, channel_username, channel_name, channel_url FROM forced_channels",
            fetch="all"
        )

    def add_forced_channel(self, ctype, ch_id, uname, name, url):
        self._execute(
            "INSERT INTO forced_channels (channel_type, channel_id, channel_username, channel_name, channel_url) VALUES (?,?,?,?,?)",
            (ctype, ch_id, uname, name, url)
        )
    ...
    def get_tasks(self):
        return self._execute(
            "SELECT id, task_type, channel_id, channel_username, channel_name, channel_url, reward FROM tasks",
            fetch="all"
        )

    def add_task(self, ttype, ch_id, uname, name, url, reward=TASK_REWARD):
        self._execute(
            "INSERT INTO tasks (task_type, channel_id, channel_username, channel_name, channel_url, reward) VALUES (?,?,?,?,?,?)",
            (ttype, ch_id, uname, name, url, reward)
        )
    ...
    def search_user(self, query):
        try:
            q = int(query)
            row = self._execute(
                "SELECT user_id, username, first_name, successful_invites, stars, vip, daily_streak FROM users WHERE user_id=?",
                (q,), fetch="one"
            )
            if row: return [row]
        except: pass
        return self._execute(
            "SELECT user_id, username, first_name, successful_invites, stars, vip, daily_streak FROM users WHERE username LIKE ? OR first_name LIKE ? LIMIT 10",
